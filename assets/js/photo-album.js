  ...
  document.querySelectorAll('[data-photo-album]').forEach((album) => {
    ...
    const show = async (position) => {
      ...
      try {
        ...
        index = position;
        album.querySelector('[data-album-caption]').textContent = photos[index].caption;
        ...
        status.textContent = `Photograph ${index + 1} of ${photos.length}. ${photos[index].caption}`;
        preloadNext();
      } catch {
        status.textContent = 'This photograph could not be loaded. Please try again.';
      } finally {
        busy = false;
