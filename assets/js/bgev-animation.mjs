...
function render(){
 ...
 if(stage!==s){$('h2').textContent=titles[s];$('.bgev-step').textContent=`0${s+1} / 04`;$('.bgev-equations').innerHTML=equations[s];stage=s;}
 ...
 if(spatial){$('.bgev-site-copy').innerHTML=`<strong>Location ${'ABC'[site]} → orange density</strong><br>μ = ${p.mu.toFixed(2)} · ϱ = ${p.rho.toFixed(2)}<br>ξ = ${p.xi.toFixed(2)} · δ = ${p.delta.toFixed(2)}<br><span class="bgev-sub">Illustrative simulation · seed 2026</span>`;$$('[data-site]').forEach(b=>b.setAttribute('aria-pressed',Number(b.dataset.site)===site));}
 ...
 $('.bgev-note').textContent=(s===1||s===2?'Quantile rail: p = 0.1, 0.3, 0.5, 0.7, 0.9. ':'')+`Fixed window −4 ≤ y ≤ 8; ${mass.toFixed(2)}% probability outside. `+(p.xi===-1?`Left density limit at y = 1: ${(p.delta+1).toFixed(2)}; density is zero beyond support.`:'Curves are not renormalized.');
 ...
 $('.bgev-progress').value=time;$('.bgev-time').textContent=`${time.toFixed(1)} / 18 s`;$('.bgev-play').textContent=playing?'Pause':'Play';
}
